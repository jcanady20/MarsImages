...
namespace MarsImages.Internal.Services
{
    public class PhotoCacheService : IPhotoCacheService
    {
        ...
        public PhotoCacheService(ILogger<PhotoCacheService> logger, IConfiguration configuration, IMemoryCache memoryCache, IImageMetaDataService imageMetaDataService, IImageService imageService)
        {
            _logger = logger;
        ...
        private async Task CacheImagesAsync(Data.Models.ImageDate imageDate)
        {
            ...
            var tasks = imageDate.Photos.Select(async photo => {
                ...
                photo.LocalSource = await _imageService.CacheMarsImageAsync(photo);
